...
def get_incidence_absolute(r_data, r2_data):
    specific = 0
    non_specific = 0
    no_fc = 0
    for i in range(len(r_data)):
        ...
        if -0.1 < r_data[i] < r_threshold and -r_threshold < r2_data[i] < r_threshold:
            no_fc += 1
    spurious_fc = len(r_data) - (specific + non_specific + no_fc)
    return np.array([specific, spurious_fc, non_specific, no_fc])
...
data_x = [corrs_r_matlab_qc_gsr, corrs_r_mdma_all_gsr, corrs_r_marciano_gsr]
data_y = [corrs_r2_matlab_qc_gsr, corrs_r2_mdma_all_gsr, corrs_r2_marciano_gsr]
...
incidence_all_fisher = []
#get incidence for fisher
for i in range(len(data_x)):
    incidence = get_incidence_absolute(data_x[i], data_y[i])
    incidence_fisher = [incidence[0], incidence[1] + incidence[2]]
    incidence_all_fisher.append(incidence_fisher)

for i in range(len(incidence_all_fisher)):
    print(incidence_all_fisher[i][0]/np.sum(incidence_all_fisher[i])*100)
...
p_values.append(fisher_exact([incidence_all_fisher[0], incidence_all_fisher[1]])[1])  # A vs B
p_values.append(fisher_exact([incidence_all_fisher[0], incidence_all_fisher[2]])[1])  # A vs C
p_values.append(fisher_exact([incidence_all_fisher[1], incidence_all_fisher[2]])[1])  # B vs C
...
y_max = 100
y_bar = y_max + 8  # height of the significance bar
h = 3
y_bar2 = y_max + 2
h2 = 3
...
for label, fc in all_fc_short.items():
    p = ax.bar(x_labels, fc, width=0.65, label=label, bottom = bottom)
    bottom += fc
ax.plot([x[0], x[0], x[2], x[2]], [y_bar, y_bar+h, y_bar+h, y_bar], lw=1, c='k')
ax.text((x[0] + x[2]) / 2, y_bar + h/1.8, '**', ha='center', va='bottom', fontsize=10)

ax.plot([x[0], x[0], x[1], x[1]], [y_bar2, y_bar2+h2, y_bar2+h2, y_bar2], lw=1, c='k')
ax.text((x[0] + x[1]) / 2, y_bar2 + h2/1.8, '*', ha='center', va='bottom', fontsize=10)

ax.legend(fontsize = 8, markerscale = 0.75, handlelength = 0.9, loc='lower center', bbox_to_anchor=(0.5, 0.99), ncol=2, frameon=False)
...
data_x_kde = [corrs_r_matlab_qc_gsr, corrs_r_mdma_all_gsr, corrs_r_marciano_gsr]
data_y_kde = [corrs_r2_matlab_qc_gsr, corrs_r2_mdma_all_gsr, corrs_r2_marciano_gsr]
kdes_x = []
kdes_y = []
